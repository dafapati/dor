...
class XL(Config):
    ...
    def reqPassword(self):
        ...
        try:
            if(status['SOAP-ENV:Envelope']['SOAP-ENV:Body'][0]['ns0:CommonResponse'][0]['ns0:ErrorMessage'] == ['SUCCESS_FORGOT_PASSWORDMYXL_NEW']):
                return {"message" : "\033[0;33m["+p+"+"+kt+"] "+g+"Successfully get Password"}
            else: 
                return {"message" : "\033[0;33m["+p+"!"+kt+"] "+m+"Failed get Password"}
        except:
    ...
    def purchasePackage(self, serviceid):        
        ...
        status = json.loads(r.content)
        if(len(status) == 4): return {"message" : "\033[0;33m["+p+"+"+kt+"] "+g+"Successfully purchased the package"}
        else: return {"message" : status['\033[0;33m['+p+'!'+kt+'] '+g+'message']}
